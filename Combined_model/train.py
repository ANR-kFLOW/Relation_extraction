...
import nltk
nltk.download('punkt')
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, GenerationConfig, PretrainedConfig  # , BertTokenizerFast
import torch
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to pretrained model config or model config identifier from huggingface.co/models.",
        required=False,
    )
...
def extract_triplets(texts, gold_extraction, prediction=False, flag=False):
    triplets = []
    #print(len(texts))
    for text in texts:
        #print('seperate')
        ...
        try:
            ...
            if flag == True:
                print('text')
                print(text)
                print('what is read as triplet')
                print(text.count(a))
                print('what the split looks like')
                print(text.split(a))
                parts = text.split(a)
                print('what the end is supposed to look like')
                s = a.join(parts[:2])
                print(s)
                print('end of block')
            obj_tag = '<obj>'
            if text.count('<') > 3:
                ...
                first_angle_after_obj = text.find('<', obj_index + len(obj_tag))
                if first_angle_after_obj != -1:
                    text = text[:first_angle_after_obj]
            if text.count(a) > 1:
                parts = text.split(a)
                #print(text)
                #print('here')
                ...
                text = a.join(parts[:2])
            if flag == True:
                print('this is supposed to be good')
                print(text)
...
def test_model(data, model, args):
    ...
    gt = []
    texts=[]
    model_config = PretrainedConfig.from_pretrained(args.config)
    ...
    for val_data, val_label in test_dataloader:
        ...
        #outputs = model.generate(input_id,config)
        outputs = model.generate(input_id,config)
        #print(len(outputs))
        outputs = tokenizer.batch_decode(outputs, skip_special_tokens=False)
        ...
        #print('group start')
        #print('gt')
        #print(extract_triplets(labels, gold_extraction=True))
        #print('pred')
        #print(extract_triplets(outputs, gold_extraction=False))
        #print('text')
        #print(get_text_from_ids(input_id,tokenizer))
        #print('group end')
        #a = len(extract_triplets(labels, gold_extraction=True))
        #b = len(extract_triplets(outputs, gold_extraction=False))
        #c = len(get_text_from_ids(input_id,tokenizer))
        a = 0
        b = 0
        c = 0
        ...
        if len(extract_triplets(outputs, gold_extraction=False)) > 4:
            print(extract_triplets(outputs, gold_extraction=False,flag=True))
            print(len(extract_triplets(outputs, gold_extraction=False)))
            print(outputs)
    ...
    # Save to CSV
    pred_df.to_csv(f'rebel_prediction/pred_rebel_{datetime.now()}.csv', index=False)
    pred_df.to_csv('rebel_prediction/pred_rebel.csv', index=False)
    gt_df.to_csv('rebel_prediction/gt.csv', index=False)
    # scores, precision, recall, f1 = re_score(pred, gt, 'relation')
    # scores, precision, recall, f1 = re_score(pred, gt, 'subject')
    # scores, precision, recall, f1 = re_score(pred, gt, 'object')
...
def make_predictions(texts, path_to_model):
    ...
    model = torch.load(path_to_model,map_location=torch.device('cpu')).to(device)
    model.eval()
...
if __name__ == "__main__":
    ...
    model_checkpoint = args.model_type
    model = AutoModelForSeq2SeqLM.from_pretrained(model_checkpoint)
    ...
    with open(args.model_name_or_path, 'rb') as f:
       buffer = io.BytesIO(f.read())

    # Load the model from the buffer
    model = torch.load(buffer,map_location=torch.device('cpu')).to(device)
    test_model(test_data, model, args)
