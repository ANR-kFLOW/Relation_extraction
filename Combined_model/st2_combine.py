...
def main():
    ...
    if args.do_test:
        ...
        if args.do_train: # continuing from above
            ...
            model.load_state_dict(torch.load(args.load_checkpoint_for_test, map_location=torch.device('cpu')))
        ...
        with open(os.path.join(args.output_dir,f"test-submission-temp.json"), "w") as f:
            for i, prediction in enumerate(predictions):
                f.write(json.dumps({"index": i, "prediction": prediction}) + "\n")
