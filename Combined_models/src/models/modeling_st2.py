...
#        self.model = AutoModel.from_pretrained('/data/Youss/RE/CausalNewsCorpus/outs/baseline/firstFT/epoch_1')
#        self.tokenizer = AutoTokenizer.from_pretrained('albert-base-v2')

        #self.config = AutoConfig.from_pretrained(args.st2_model_name_or_path)
...
#
#        # Calculate the number of layers to freeze (50% of total layers)
...
#            num_layers_frozen += 1
#            if num_layers_frozen >= num_layers_to_freeze:
#                break

        classifier_dropout = self.args.st2_dropout
        ...
        if self.args.st2_signal_classification and not self.args.st2_pretrained_signal_detector:
    ...
    def forward(
        ...
        if signal_bias_mask is not None and not self.args.st2_signal_bias_on_top_of_lm:
            ...
            if self.args.st2_model_name_or_path in ['facebook/bart-large', 'facebook/bart-base', 'facebook/bart-large-cnn']:
                outputs = self.model(
                    input_ids,
                    attention_mask=attention_mask,
                    head_mask=head_mask,
            ...
            elif self.args.st2_model_name_or_path in ['microsoft/deberta-base']:
                outputs = self.model(
                    input_ids,
                    attention_mask=attention_mask,
                    token_type_ids=token_type_ids,
        ...
        if signal_bias_mask is not None and self.args.st2_signal_bias_on_top_of_lm:
        ...
        start_arg0_logits, end_arg0_logits, start_arg1_logits, end_arg1_logits, start_sig_logits, end_sig_logits = logits.split(
            1, dim=-1)
    ...
    def signal_phrases_bias(self, input_ids, signal_bias_mask):
        ...
        inputs_embeds[
            signal_bias_mask == 1] += self.signal_phrases_layer  # self.signal_phrases_layer(inputs_embeds[signal_bias_mask == 1])
            ...
            if eval(index)[2] == 'before':
                ...
                for m in range(len(start_cause_values)):
                    for n in range(len(end_effect_values)):
                        topk_scores[str((start_cause_indices[m].item(), end_cause, start_effect,
                                         end_effect_indices[n].item()))] = score + start_cause_values[m].item() + \
                                                                           end_effect_values[n].item()
                ...
                for m in range(len(end_cause_values)):
                    for n in range(len(start_effect_values)):
                        topk_scores[str((start_cause, end_cause_indices[m].item(), start_effect_indices[n].item(),
                                         end_effect))] = score + end_cause_values[m].item() + start_effect_values[
                            n].item()
