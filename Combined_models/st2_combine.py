...
def parse_args():
    ...
    if args.st2_push_to_hub:
        assert args.st2_output_dir is not None, "Need an `output_dir` to create a repo when `--st2_push_to_hub` is passed."
...
def main_st2(arg):
    ...
    if args.st2_signal_classification and args.st2_pretrained_signal_detector:
        signal_detector = SignalDetector(args.st2_signal_model_and_tokenizer_path)

    if args.st2_seed is not None:
        set_seed(args.st2_seed)

    # Handle the repository creation
    if accelerator.is_main_process:
        if args.st2_push_to_hub:
            if args.st2_hub_model_id is None:
                repo_name = get_full_repo_name(Path(args.st2_output_dir).name, token=args.st2_hub_token)
            else:
                repo_name = args.st2_hub_model_id
            repo = Repository(args.st2_output_dir, clone_from=repo_name)

            with open(os.path.join(args.st2_output_dir, ".gitignore"), "w+") as gitignore:
                if "step_*" not in gitignore:
                    gitignore.write("step_*\n")
                if "epoch_*" not in gitignore:
                    gitignore.write("epoch_*\n")
        elif args.st2_output_dir is not None:
            os.makedirs(args.st2_output_dir, exist_ok=True)
    ...
    if args.st2_dataset_name is not None:
        # Downloading and loading a dataset from the hub.
        raw_datasets = load_dataset(args.st2_dataset_name, args.st2_dataset_config_name)
    else:
        data_files = {}
        if args.st2_train_file is not None:
    ...
    if args.st2_augmentation_file is not None:
        augment_dataset = Dataset.from_pandas(pd.read_csv(args.st2_augmentation_file))
    ...
    tokenizer_name_or_path = args.st2_tokenizer_name if args.st2_tokenizer_name else args.st2_model_name_or_path
    if not tokenizer_name_or_path:
        raise ValueError(
            "You are instantiating a new tokenizer from scratch. This is not supported by this script."
            "You can do it from another script, save it, and load it from here, using --st2_tokenizer_name."
        )
    ...
    def tokenize_and_align_labels(examples):
        tokenized_inputs = tokenizer(
            examples['tokens'],
            max_length=args.st2_max_length,
            padding=padding,
            truncation=True,
        ...
        for i, tokens in enumerate(examples['tokens']):
            ...
            if word2tok:
                last_key, last_value = list(word2tok.items())[-1]
                first_key, first_value = list(word2tok.items())[0]
                # print(last_value)
                if last_value == []:
                    # print('yes')
                    word2tok[last_key] = [0]
    ...
    if args.st2_train_file is not None:
        with accelerator.main_process_first():
            ...
            if args.st2_max_train_samples is not None:
    ...
    if train_dataset is not None:
        train_dataloader = DataLoader(
            train_dataset, shuffle=True, collate_fn=data_collator, batch_size=args.st2_per_device_train_batch_size
        )
    else:
        train_dataloader = None
    if eval_dataset is not None:
        eval_dataloader = DataLoader(
            eval_dataset, shuffle=False, collate_fn=data_collator, batch_size=args.st2_per_device_eval_batch_size
        )
    else:
        eval_dataloader = None
    if test_dataset is not None:
        test_dataloader = DataLoader(
            test_dataset, shuffle=False, collate_fn=data_collator, batch_size=args.st2_per_device_test_batch_size
        )
    else:
        test_dataloader = None

    if args.st2_do_train:
        # Optimizer
        ...
        optimizer_grouped_parameters = [
            {
                ...
                "weight_decay": args.st2_weight_decay,
            },
            {
        ...
        num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.st2_gradient_accumulation_steps)
        if args.st2_max_train_steps is None:
            args.st2_max_train_steps = args.st2_num_train_epochs * num_update_steps_per_epoch
            overrode_max_train_steps = True

        lr_scheduler = get_scheduler(
            name=args.st2_lr_scheduler_type,
            optimizer=optimizer,
            num_warmup_steps=args.st2_num_warmup_steps,
            num_training_steps=args.st2_max_train_steps,
        )
        ...
        args.st2_num_train_epochs = math.ceil(args.st2_max_train_steps / num_update_steps_per_epoch)

        # Figure out how many steps we should save the Accelerator states
        if hasattr(args.st2_checkpointing_steps, "isdigit"):
            checkpointing_steps = args.st2_checkpointing_steps
            if args.st2_checkpointing_steps.isdigit():
                checkpointing_steps = int(args.st2_checkpointing_steps)
        else:
        ...
        progress_bar = tqdm(range(args.st2_max_train_steps), disable=not accelerator.is_local_main_process)
        completed_steps = 0
        starting_epoch = 0
        # Potentially load in the weights and states from a previous save
        if args.st2_resume_from_checkpoint:
            if args.st2_resume_from_checkpoint is not None or args.st2_resume_from_checkpoint != "":
                accelerator.print(f"Resumed from checkpoint: {args.st2_resume_from_checkpoint}")
                accelerator.load_state(args.st2_resume_from_checkpoint)
                path = os.path.basename(args.st2_resume_from_checkpoint)
            else:
        ...
        for epoch in range(starting_epoch, args.st2_num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                loss = loss / args.st2_gradient_accumulation_steps
                accelerator.backward(loss)
                if step % args.st2_gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                    optimizer.step()
                    lr_scheduler.step()
                    optimizer.zero_grad()
            ...
            for step, batch in enumerate(eval_dataloader):
                ...
                for i in range(len(batch["input_ids"])):
                    ...
                    if args.st2_postprocessing_position_selector:
