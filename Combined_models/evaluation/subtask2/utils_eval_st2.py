...
def main(ref_df, pred_list, calculate_best_combi=True):
    ...
    for i in regular_ids:
        ...
        if len(ce_ref) != len(ce_pred):
            min_len = min(len(ce_ref), len(ce_pred))
            ce_ref = ce_ref[:min_len]
            ce_pred = ce_pred[:min_len]

        if len(sig_pred) != len(sig_ref):
            min_len = min(len(sig_pred), len(sig_ref))
            sig_pred = sig_pred[:min_len]
            sig_ref = sig_ref[:min_len]

        assert(len(ce_ref)==len(ce_pred))
        assert(len(sig_pred)==len(sig_ref))
        metric.add(
            prediction=ce_pred,
            reference=ce_ref 
        )
    ...
    if grouped_df is not None:
        ...
        for _, row in grouped_df.iterrows():
            ...
            for a,b in list(best_pair):
                _, ce_ref, sig_ref = refs[a]
                _, ce_pred, sig_pred = preds[b]
                print(len(ce_pred)==len(ce_ref))
                print(len(sig_ref)==len(sig_pred))
